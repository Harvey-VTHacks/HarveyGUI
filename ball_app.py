...
class StandaloneBall(QWidget):
    ...
    def start_spinning(self):
        """Start the ball spinning and scale it up"""
        print("Starting ball spinning...")
        self.is_spinning = True
        self.target_scale = self.large_size / self.original_size  # Scale to larger size
        self.timer.start(16)  # ~60 FPS
        
    def stop_spinning(self):
        """Stop the ball spinning and scale it back to original size"""
        print("Stopping ball spinning...")
    ...
    def mouseReleaseEvent(self, event):
        ...
        if event.button() == Qt.LeftButton and self.dragging:
            ...
            if self.drag_start_position:
                drag_distance = (event.globalPos() - (self.frameGeometry().topLeft() + self.drag_start_position)).manhattanLength()
